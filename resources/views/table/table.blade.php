...
@section('service')
    <ul id="sidebarnav" class="pt-4" style="background-color: #0E566E;">
        @foreach ($request as $requests2)
            <li class="sidebar-item">
                <a class="sidebar-link waves-effect waves-dark sidebar-link"
                    href="/collection/{{ $collection->id }}/table/{{ $requests2->method_request }}" aria-expanded="false"
                    style="text-decoration: none">
                    <?php
                    if($requests2->method_request == "GET"){
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: #54FD46">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>

                    <?php
                    }else if($requests2->method_request == "POST") {
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: #e1ff35">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>

                    <?php
                    }else if($requests2->method_request == "PUT") {
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: rgb(47, 103, 255)">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>

                    <?php
                    }else if($requests2->method_request == "PATCH") {
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: rgb(199, 77, 255)">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>

                    <?php
                    }else if($requests2->method_request == "DELETE") {
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: rgb(255, 157, 0)">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>

                    <?php
                    }else if($requests2->method_request == "HEAD") {
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: rgb(60, 255, 0)">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>

                    <?php
                    }else if($requests2->method_request == "OPTIONS") {
                        ?>
                    <i class="hide-menu" style="font-size:20px; color: pink">{{ $requests2->method_request }}
                        <span style="font-size:20px; color:ghostwhite">{{ $requests2->request_name }}</span>
                    </i>
...
@section('content')
    ...
    <div class="card-body">
        {{-- Method UR --}}
        @php $loopCount = 0; @endphp <!-- เริ่มต้นการนับลูป -->

        @foreach ($request2 as $requests)
            @if ($loopCount == 1)
            @break
        @endif
                ...
                <tr>
                    <th style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                        <select name="required_param" id="required_param"
                            ...
                            <option value="{{ $requests->method_request }}" @required(true)>
                                {{ $requests->method_request }}</option>
                    ...
                    <th id="rount" style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                        {{ $requests->method_url }}</th>
                </tr>
            </tbody>
        </table>
        @php $loopCount++; @endphp <!-- เพิ่มการนับลูปขึ้นไป 1 -->
    @endforeach
...
<div style="margin-left: 15px; width:1150px">
    ...
    <label for="header" style="font-size:14px; color:white">Headers
        &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</label>
    <input type="checkbox" id="cb_param" name="cb_param">
    <label for="param" style="font-size:14px; color:white">Params
        &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</label>
    <input type="checkbox" id="cb_body" name="cb_body">
    <label for="body" style="font-size:14px; color:white">Body
        &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</label>
...
<div class="card-body" id="table-container-header" style="display: none">
        ...
        <tbody id="tbody_header">
            <tr id="tr_header">
                ...
                <input type="checkbox" name="checkHeader" id="checkHeader"
                    style="margin-left: 18px; margin-top: 80px">
            </tr>
        </tbody>
    </table>
    <br>
    <div class="custom-div" style="width: 1180px; border: 1px solid dimgray; border:none">
        ...
        for (let i = 0; i < table.rows[0].cells.length; i++) { const newCell=document.createElement("th");
            newRow.appendChild(newCell); } // Append the new row to the table table.appendChild(newRow); } function
            copyRow(row) { // Create a new row element const newRow=document.createElement("tr"); // Copy the cells from
            the old row to the new row for (let i=0; i < row.cells.length; i++) { const
            newCell=document.createElement("th"); newCell.style.backgroundColor = '#9DC8D6'; newCell.id = 'th_header';
            newCell.innerHTML=row.cells[i].innerHTML; newRow.appendChild(newCell); //สร้าง icon ตาม row ที่เพิ่ม if
            (i===row.cells.length - 1) { const icon=document.createElement('i'); icon.className = 'bi-trash';
            icon.onclick=function() { deleteRow(newRow); // Call deleteRow function when icon is clicked };
            newCell.appendChild(icon); } if (i===row.cells.length - 1) { const checkbox=document.createElement('input');
            checkbox.type = 'checkbox'; checkbox.name = 'checkHeader'; // Set name attribute as needed
            checkbox.style.marginLeft = '18px'; // Apply styles as desired newCell.appendChild(checkbox); // Append
            checkbox to the new cell } } // Append the new row to the table row.parentNode.appendChild(newRow); } // Add
            an event listener to the "Copy" button document.getElementById("add_row_header").addEventListener("click",
            function() { const row=document.getElementById("tr_header"); copyRow(row); const target=event.target; });
            $("table").on("click", ".bi-trash" , function() { const row=$(this).closest("tr"); // ค้นหาแถวที่ไอคอนอยู่
            // ลบแถว row.remove(); }); </script>
    </div>

    </th>
    <th id="th_header" style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6;  text-align: center">
        <textarea class="form-control" name="description_header1" id="description_header" rows="1"
            style="font-size:14px; border:none; background:#9DC8D6; width:720px"></textarea>
    ...
    <tr>
        ...
        <th style="width: 900x; font-size:14px; color:white; background-color: #032A37">
            <center>Description
        </th>
    </tr>
    </thead>
        ...
        <tr>
            ...
            <th style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                <textarea class="form-control" name="description_param" id="description_param" rows="1"
                    style="font-size:14px; border:none; background:#9DC8D6; width:530px"></textarea>
                <script>
    ...
    <th style="font-size:14px; border-color:#073B4C; background-color: #073B4C; text-align: center">
        <a class="btn " style="width:1050px;background-color:#073B4C;margin-left: ;"href="javascript:void(0)"
            data-filter="4"></a>
        <button type="button" id="add_row_response"
...
<div class="card-body" id="table-container-response" style="display: none">
    <table class="table table-bordered" name="response_table" id="response_table"
        style="width:100px; margin-left: 32px; border:dimgray">
        <thead>
            <br>
            ...
            <tr>
                <th style="width: 9%; font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                    <input type="text" name="statuscode" id="statuscode"
                        style="background-color: #9DC8D6; width:55% ; border: none">
                    <script>
        ...
        <table class="table table-bordered" name="response_table" id="response_table"
            style="width:957px; margin-left: 25px; border:dimgray">
            <thead>
                <tr>
                    <th style="width: 9%; font-size:14px; color:white; background-color: #032A37">
                        <center>Key
                    </th>
                    <th style="width: 12%; font-size:14px; color:white; background-color: #032A37">
                        <center>Data Type
                    </th>
                    <th style="width: 15%; font-size:14px; color:white; background-color: #032A37">
                        <center>Example Data
                    </th>
                    <th style="font-size:14px; color:white; background-color: #032A37">
                        <center>Description
            ...
            <tbody id="my_tbody">
                <tr>
                    <th style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                        <input type="text" name="key_response" id="key_response"
                            style="background-color: #9DC8D6; width:100%; border: none">
                        <script>
                    ...
                    <th style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                        <select name="datatype_body" id="datatype_response"
                            style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6; border: none">
                            <option value="boolean">Boolean</option>
                            <option value="integer">Integer</option>
                            <option value="number">Number</option>
                    ...
                    <th style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                        <input type="text" name="exampledata_response" id="exampledata_response"
                            style="background-color: #9DC8D6; width:100%; border: none">
                        <script>
                    ...
                    <th style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6">
                        <input type="text" name="description_response" id="description_response"
                            style="background-color: #9DC8D6; width:100%; border: none">
                        <script>
