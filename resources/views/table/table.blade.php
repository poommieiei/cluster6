            ...
            <div class="custom-div" style="width: 1180px; border: 1px solid dimgray; border:none">
                <div style="display: flex; justify-content: flex-end;">
                    <button type="button" id="add_row_response"
                        style="color: #F0FFFF; background-color: Black; width: 8%;">
                ...
                for (let i = 0; i < table.rows[0].cells.length; i++) { const newCell=document.createElement("th");
                    ...
                    copyRow(row) { // Create a new row element const newRow=document.createElement("tr"); // Copy the cells
                    from the old row to the new row for (let i=0; i < row.cells.length; i++) { const
                    newCell=document.createElement("th"); newCell.style.backgroundColor = '#9DC8D6';
                    newCell.id = 'th_header'; newCell.innerHTML=row.cells[i].innerHTML; newRow.appendChild(newCell);
                    //สร้าง icon ตาม row ที่เพิ่ม if (i===row.cells.length - 1) { const icon=document.createElement('i');
                    icon.className = 'bi-trash'; icon.onclick=function() { deleteRow(newRow); // Call deleteRow function
                    when icon is clicked }; newCell.appendChild(icon); } if (i===row.cells.length - 1) { const
                    checkbox=document.createElement('input'); checkbox.type = 'checkbox'; checkbox.name = 'checkHeader';
                    // Set name attribute as needed checkbox.style.marginLeft = '18px'; // Apply styles as desired
                    newCell.appendChild(checkbox); // Append checkbox to the new cell } } // Append the new row to the table
                    row.parentNode.appendChild(newRow); } // Add an event listener to the "Copy" button
                    document.getElementById("add_row_header").addEventListener("click", function() { const
                    row=document.getElementById("tr_header"); copyRow(row); const target=event.target; });
                    $("table").on("click", ".bi-trash" , function() { const row=$(this).closest("tr"); //
                    ค้นหาแถวที่ไอคอนอยู่ // ลบแถว row.remove(); }); </script>
            </div>
