   ...
   <div class="custom-div" style="width: 1180px; border: 1px solid dimgray; border:none">
    ...
    <script>
        function addRow() {
            ...
            table_header.appendChild(newRow_header);
            }
        function copyRow(row) {
            ...
            $("table_header").on("click", ".bi-trash", function() {
            const row_header = $(this).closest("tr"); // ค้นหาแถวที่ไอคอนอยู่
...
<div class="card-body" id="table-container-param" style="display: none">
    <table class="table table-bordered" name="param_table" id="param_table" style="width:1150px; border:dimgray; margin-left: 32px">
        <thead>
            <br>
            ...
            <tr>
                ...
                <th style="width: 150px; font-size:14px; color:white; background-color: #032A37;"><center>Param Type</th>
                ...
                <th style="width: 900px; font-size:14px; color:white; background-color: #032A37"><center>Description</th>
                <th style="background: #073B4C; border-top: solid #073B4C; border-top: solid #073B4C; border-right: solid #073B4C; border-bottom: solid #073B4C; width:50px"></th>
            </tr>
        </thead>
        <tbody id="tbody_param">
            <tr id="tr_param">
                ...
                <th id="th_param" style="font-size:14px; color:rgb(0, 0, 0); background-color: #9DC8D6; text-align: center">
                    <textarea class="form-control" name="description_param" id="description_param" rows="1" style="font-size:14px; border:none; background:#9DC8D6; width:620px"></textarea>
                    <script>
    ...
    <script>
        function addRow() {
            // Get the table element
            const table_param = document.getElementById("param_table");
            const tr_param = document.getElementById("tr_param");

            // Create a new row element
            const newRow_param = document.createElement("tr");

            // Create a new cell element for each column in the table
            for (let i = 0; i < table.rows[0].cells.length; i++) {
                const newCell_param = document.createElement("th");
                newRow_param.appendChild(newCell_param);
            }
            // Append the new row to the table
            table_param.appendChild(newRow_param);
            }
        function copyRow(row) {
            // Create a new row element
            const newRow_param = document.createElement("tr");
            ...
            for (let i = 0; i < row.cells.length; i++) {
                const newCell_param = document.createElement("th");
                newCell_param.id = 'th_param';
                newCell_param.style.backgroundColor = '#9DC8D6';
                newCell_param.innerHTML = row.cells[i].innerHTML;
                newRow_param.appendChild(newCell_param);
                //สร้าง icon ตาม row ที่เพิ่ม
                if (i === row.cells.length - 1) {
                    const icon_param = document.createElement('i');
                    icon_param.className = 'bi-trash';
                    icon_param.style.fontSize = '22px';
                    icon_param.style.color = '#F0FFFF';
                    newCell_param.style.backgroundColor = 'transparent';
                    newCell_param.style.borderRight = 'solid #073B4C';
                    newCell_param.style.borderBottom = 'solid #073B4C';
                    icon_param.onclick = function() {
                        deleteRow(newRow_param);
                    };
                    newCell_param.appendChild(icon_param);
                }
            }
            //
            row.parentNode.appendChild(newRow_param);
            }

            // Add an event listener to the Copy button
            document.getElementById("add_row_param").addEventListener("click", function() {
            const row_param = document.getElementById("tr_param");
            copyRow(row_param);
            const target = event.target;
            });
            $("table").on("click", ".bi-trash", function() {
            const row_param = $(this).closest("tr"); // ค้นหาแถวที่ไอคอนอยู่
...
<div class="card-body" id="table-container-body" style="display: none">
    ...
    <script>
        function addRowBody() {
            // Get the table element
            const table_header = document.getElementById("body_table");
            const tr_header = document.getElementById("tr_body");
            table_header.style.display = 'nline-block';

            // Create a new row element
            const newRow_body = document.createElement("tr");

            // Create a new cell element for each column in the table
            for (let i = 0; i < table.rows[0].cells.length; i++) {
                const newCell_header = document.createElement("th");
                newRow_body.appendChild(newCell_header);
            }
            // Append the new row to the table
            table_header.appendChild(newRow);
            }
        function copyRowBody(row) {
            // Create a new row element
            const newRow_body = document.createElement("tr");
            // Copy the cells from the old row to the new row
            for (let i = 0; i < row.cells.length; i++) {
                const newCell_body = document.createElement("th");
                newCell_body.id = 'th_body';
                newCell_body.style.backgroundColor = '#9DC8D6';
                newCell_body.innerHTML = row.cells[i].innerHTML;
                newRow_body.appendChild(newCell_body);
                //สร้าง icon ตาม row ที่เพิ่ม
                if (i === row.cells.length - 1) {
                    const icon_body = document.createElement('i');
                    icon_body.className = 'bi-trash';
                    icon_body.style.fontSize = '22px';
                    icon_body.style.color = '#F0FFFF';
                    newCell_body.style.backgroundColor = 'transparent';
                    newCell_body.style.borderTop = 'solid #073B4C';
                    newCell_body.style.borderRight = 'solid #073B4C';
                    newCell_body.style.borderBottom = 'solid #073B4C';
                    icon_body.onclick = function() {
                        deleteRow(newRow_body);
                    };
                    newCell_body.appendChild(icon_body);
                }
            }
            //
            row.parentNode.appendChild(newRow_body);
            }

            // Add an event listener to the Copy button
            document.getElementById("add_row_body").addEventListener("click", function() {
            const row_body = document.getElementById("tr_body");
            copyRowBody(row_body);
            const target = event.target;
            });
            $("table").on("click", ".bi-trash", function() {
            const row_body = $(this).closest("tr"); // ค้นหาแถวที่ไอคอนอยู่
